...
function LandingPageModel({ isOpen, onClose, image, techData }) {

  console.log("techongluimage--",image);
...
const DATE_FORMAT_OPTIONS = {
  ...
  //console.log("extractedDatalandinpage--", extractedData);
  
  useEffect(() => {
    const processTechnologies = () => {
      if (techData && image?.id && techData[image.id]) {
        return techData[image.id]
          .flatMap(item => item?.website_technology_mapping?.website_technologies || [])
          .map(tech => ({
            id: tech?.id,
            name: tech?.name,
            logoUrl: tech?.logo?.url ? `${BASE_URL}${tech.logo.url}` : null
    ...
    const technologies = processTechnologies();
    
  }, [techData, image]);
  
  if (!isOpen || !image || !techData) return null;
    ...
    >
      ...
      >
        <div className="flex p-1 sm:p-4">
          <div>
            {/* <Image
              src={image.clientLogo || DEFAULT_IMAGE}
              alt={image.clientName || "Client Name"}
              width={40}
                height={20}
                onError={(e) => {
        ...
        <div className="font-semibold xs:space-y-3 mt-3 xs:mt-6 text-xs sm:text-lg">
          <h1>
            ...
            <span className="font-thin">
              {image.launch_date? new Date(image.launch_date)
                .toLocaleDateString("en-GB", DATE_FORMAT_OPTIONS)
                .replace(/(\d{2})-(\w{3})-(\d{4})/, "$1-$2-$3") : "Date not available"}
            </span>
          </h1>
          <h1>
            Website Link :{" "}
...
export default memo(LandingPageModel)