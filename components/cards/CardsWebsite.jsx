...
function CardsWebsite() {
  const { websitesData, selectedCountry, isLoading, error,searchTerm } = useWebsite();
  //console.log("websitesData---",websitesData?.data);
  ...
  // console.log(
  //   "websitesData--",
  //   websitesData?.data?.map((item) => item.name)
  // );
  // console.log("isLoading--", isLoading);

  // Process websites data when it changes
  useEffect(() => {
    if (websitesData?.data) {
      const extractedUrls = websitesData.data.map((item) => ({
        id: item.id,
        name: item.name,
        website_url: item.website_url,
        launch_date: item.launch_date,
        url: item.thumbnail?.url ? `${BASE_URL}${item.thumbnail.url}` : "",
  ...
  if (!websitesData || !websitesData.data || websitesData.data.length === 0) {return <div>No data available</div>};
  ...
  return (
    <>
      ...
      {imageUrls.length > 0 ? (
          imageUrls
            // 🔥 Filter by search term (matches name or client name)
            .filter((item) => {
            ...
            <div key={index} className="border rounded-lg ">
              <div className="max-w-l bg-transparent border border-gray-200 rounded-lg hover:shadow-2xl">
                <a onClick={() => handleImageClick(item)}>
                  {item.url ? (
                    <div className="relative xl:h-80 h-72 w-screen sm:w-full cursor-pointer">
                      {/* <Image
                        src={item.url}
                        alt={`Website Preview ${index + 1}` || "Website Image"}
                        width={600}
                        height={400}
                        objectFit="cover"
