...
function DropdownVideos() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      setError(null);
      try {
        let url = `${BASE_URL}/api/videos?populate=*`;
        if (selectedCountry !== "No Country Selected") {
          url = `${BASE_URL}/api/videos?filters[country][name][$eq]=${selectedCountry}&populate=*`;
        }
        console.log("url---", url);
    ...
    fetchVideos();
  }, [setVideoData, selectedCountry, setIsLoading, setError]);
