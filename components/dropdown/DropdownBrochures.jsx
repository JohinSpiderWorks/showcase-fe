...
function DropdownBrochures() {
  ...
  useEffect(() => {
    const fetchBrochure = async () => {
      setIsLoading(true)
      try {
        let url = `${BASE_URL}/api/brochures?populate[brochure_pages][populate]=image_name&populate=thumbnail`
        if (selectedCountry !== "No Country Selected"){
            url = `${BASE_URL}/api/brochures?filters[country][name][$eq]=${selectedCountry}&populate=*`
        }
        console.log("urlbrochure:",url);
        
        const response = await axios.get(url)
        console.log("fetched brochure data---",response.data);
        setBrochureData(response.data)
      }
      catch(error) {
    ...
    fetchBrochure()
  },[selectedCountry,setBrochureData,setIsLoading,setError])
                ...
                <div className="absolute left-0 mt-2  z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700">
                  ...
                  <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                    {countries?.data?.map((country, index) => (
                          ...
                          onClick={() => {
                            setSelectedCountry(
                              selectedCountry === country.name ? "No Country Selected" : country.name
                            );
                            setDropdownOpen(false);
                            }}
                        >
...
export default DropdownBrochures