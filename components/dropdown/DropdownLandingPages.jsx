...
function DropdownLandingPages() {
  ...
  useEffect(() => {
    const fetchLandingPage = async () => {
      ...
      setError(null);
      try {
        let url = `${BASE_URL}/api/websites?filters[type][$eq]=Landing%20Page`;
        if (selectedCountry !== "No Country Selected") {
          url += `&filters[country][name][$eq]=${selectedCountry}`;
        }
        if (selectedTechnology !== "No Technology Selected") {
          url += `&filters[website_technologies][name][$eq]=${selectedTechnology}`;
        }
        url += `&populate=*`;
        console.log("url---landingpage", url);
        const response = await axios.get(url);
        console.log("landingpage---data-----", response.data);
...
export default DropdownLandingPages;
