...
function DropdownCreatives() {
    const { countries,isLoading } = useClient()
    const {setIsLoading,setCreativeData,selectedCountry,setError,setSelectedCountry,setSearchTerm, searchTerm} =useCreative()
   // console.log("countries--creatives",countries);
  ...
  //const [searchTerm, setSearchTerm] = useState("");
    ...
    useEffect(() => {
        const fetchCreative = async () => {
          setIsLoading(true)
          try {
            let url = `${BASE_URL}/api/creatives?populate=*`
            if (selectedCountry !== "No Country Selected"){
                url = `${BASE_URL}/api/creatives?filters[country][name][$eq]=${selectedCountry}&populate=*`
            }
            console.log("urlCreative:",url);
            
            const response = await axios.get(url)
            console.log("fetched Creative data---",response.data);
        ...
        fetchCreative()
      },[selectedCountry,setCreativeData,setIsLoading,setError])
...
export default DropdownCreatives