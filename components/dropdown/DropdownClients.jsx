...
function DropdownClients() {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      setIsLoading(true);
      try {
        let url = `${BASE_URL}/api/clients?populate=*`;
        if (selectedCountry !== "No Country Selected") {
          url = `${BASE_URL}/api/clients?filters[country][name][$eq]=${selectedCountry}&populate=*`;
        }
        console.log("url--", url);
    ...
    fetchClient();
  }, [selectedCountry, setClientData, setIsLoading, setError]);
